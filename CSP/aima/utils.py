...
# ____________________________________________________________________________
def generate_random_string_list(max_length, size):
    """
    Generate a list of random strings with a maximum length and size.
    """
    return [generate_random_string(max_length) for _ in range(size)]

# ____________________________________________________________________________
def generate_random_choices(string_list, max_size):
    """
    Generate a dictionary where the keys are elements from the list
    and the values are a random list of choices from the input list.
    The size of the value list is randomly selected between 2 and max_size.
    """
    choices_dict = {}
    for string in string_list:
        size = random.randint(2, max_size)
        choices = random.sample(string_list, size)
        choices_dict[string] = choices
    return choices_dict
# ____________________________________________________________________________
def generate_random_graph(string_list, max_size):
    """
    Generate a dictionary where the keys are elements from the list
    and the values are a random list of choices from the input list.
    The size of the value list is randomly selected between 2 and max_size.
    The resulting dictionary is interpreted as an undirected graph.
    """
    graph_dict = {}
    for string in string_list:
        size = random.randint(2, max_size)
        choices = set(random.sample(string_list, size))
        choices.discard(string)
        graph_dict[string] = list(choices)
        for choice in choices:
            if choice in graph_dict:
                graph_dict[choice].append(string)
            else:
                graph_dict[choice] = [string]
    return graph_dict
