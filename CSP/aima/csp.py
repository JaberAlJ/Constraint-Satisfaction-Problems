...
class MapColoring(CSP):
    """This class describes map coloring CSP.
    This problem specified by the following inputs:
    variables   A list of variables; each is atomic (e.g. int or string).
    domains     A dict of {var:[possible_value, ...]} entries.
    neighbors   A dict of {var:[var,...]} that for each variable lists
                the other variables that participate in constraints.

    Note, we assume that all the constraints are binary. And as defined in the
    map coloring problem, the variables should have different values.
    """

    def __init__(self, variables, domains, neighbors):
        """
        Construct a map coloring problem by calling the constructor of the CSP class.
        By default, we use the `different_values_constraint`.
        """
        super().__init__(variables, domains, neighbors, different_values_constraint)


class Cryptarithmetic(NaryCSP):
    """
    This class defines a cryptarithmetic puzzle.
    
    We create a puzzle by providing two key data structures:
    domains     : a dictionary that maps each variable to its domain
    constraints : a list of constraints
    """
